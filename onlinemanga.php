...
$dataOM=mysqli_query($conn, "select *, manga.id as mId, chapters.id as cId from manga, chapters where active=1 and manga.id=chapters.manga_id order by chapters.updated_date desc");
        ...
        <div class="container-fluid">
          <div class="row mb-2">
            <div class="col-lg-8 col-12">
              <h1 class="m-0 text-dark"> Black & White - Latest Manga</h1>
            </div><!-- /.col -->
          </div><!-- /.row -->
          <hr>
        </div><!-- /.container-fluid -->
                      ...
                      <table id="manga-table" class="table table-striped dt-table">
                          ...
                          <tbody>
                              <?php while ($manga = mysqli_fetch_assoc($dataOM)) : ?>
                                  ...
                                  <tr>
                                      <td><a href="mangaread.php?mangaId=<?php echo $manga['mId'] ?>&chapterNumber=<?php echo $manga['chapter_number'] ?>"><img src="<?php echo $manga['cover_photo']; ?>" alt="" class="round listImg" style="height: 120px; width: 85px;"></a></td>
                                      <td><a class="manga-title" href="mangaread.php?mangaId=<?php echo $manga['mId'] ?>&chapterNumber=<?php echo $manga['chapter_number'] ?>"><?php echo $manga["manga_name"] ?></a><br>Chapter - <?php echo $manga['chapter_number']."/".$manga['chapters'] ?><br>Genre - <?php echo $manga['genre']; ?></td>
